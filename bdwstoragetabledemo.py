...
# Updated by Balaram Annasamudram
...
resourcegroup_name = 'abr'+''.join(random.choice(string.ascii_lowercase + string.digits) for _ in range(6))
storageaccount_name = 'abr'+''.join(random.choice(string.ascii_lowercase + string.digits) for _ in range(6))
location = 'eastus'
...
Coffee.price = 15
table_service.insert_entity('itemstable', Coffee)
print('Created entry for a Maxwell...\n')
time.sleep(1)
...
items = table_service.query_entities('itemstable', filter="PartitionKey eq 'Coffeestore'", select='brand,flavor,price')
for item in items:
...
items = table_service.query_entities('itemstable', filter="PartitionKey eq 'carsmenu'", select='make,price')
for item in items:
